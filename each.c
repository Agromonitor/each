...
int main()
{
   ...
   for (;;) {
	   ...
	   //Delay
	   if (test_send == TRUE) {
		   sleep(1800); // Delay de 30min = 1800s
		   test_send = FALSE;
	   }
...
void setting_serial_port()
{
	/* Attempt to open the serial port */
	   if ((fd = open(MODEMSETTING, O_RDWR | O_NOCTTY | O_NDELAY)) == -1) {
...
/*
 * Separates each message into several fields and affects location structure.
 */
void parseMSG(char *msg, struct Location *location, int size)
{

	//$GPRMC,090446.000,A,3845.5509,N,00906.9152,W,0.00,129.54,080813,,,A*79
	//$GPGSV,3,1,12,05,00,000,13,01,00,000,,02,00,000,,03,00,000,*7F
...
/*
 * Sends position information to a specified WebServer
 */
void sendInfoToServer (char *message) {
...
/*
 * Builds link to use in the call of server info update according with the following format:
 * This example was made to send information through an WebService, user should adapt to their application
 */
void buildLink (struct Location *location, char * address) {

	strcpy(address, "curl http://107.170.244.39:22/");
